...
export class AlchemyAAService {
  ...
  /**
   * Initialize the AA client with Web3Auth signer
   */
  async initialize(web3AuthSigner?: SmartAccountSigner): Promise<void> {
    try {
      // Check configuration first
      checkAlchemyConfig()
      
      if (!web3AuthSigner) {
        throw new Error('Web3Auth signer is required for AA initialization')
      }
  ...
  async switchChain(newChain: AASupportedChain): Promise<void> {
    this.chain = newChain
    // Re-initialize with new chain - will need Web3Auth signer
